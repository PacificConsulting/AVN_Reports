report 50004 "Std Sales Credit Memo Copy"
{
    //RDLCLayout = './SalesReceivables/Document/StandardSalesCreditMemo.rdlc';
    //WordLayout = 'src\Reports Layout\/StandardSalesCreditMemo.docx';
    RDLCLayout = 'src\Reports Layout\/StandardSalesCreditMemo -1.rdl';
    Caption = 'Sales - Credit Memo Testing';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Cr.Memo Header")
        {
            ...
            column(CompanyGST; 'GST Reg. No. : ' + CompanyInfo."GST Registration No.")
            {
            ...
            column(YourReference; "Reference Invoice No.")
            {
            }
            ...
            column(DocumentDate; 'Date :' + Format("Document Date", 0, 4))
            {
            }
            ...
            column(Compinfo1; Compinfo1.Name)
            {

            }
            column(RecEInvoiceDetail; RecEInvoiceDetail."E-Invoice IRN No.")
            {

            }
            column(RecEInvoiceDetail1; RecEInvoiceDetail."E-Invoice QR Code")
            {
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                column(TotalAmountIncludingVAT; 0)
                {
    ...
    */
                trigger OnAfterGetRecord()//SCL
                begin

                    if RecEInvoiceDetail.Get("Document No.") then begin
                        RecEInvoiceDetail.CalcFields(RecEInvoiceDetail."E-Invoice QR Code");
                    end;
                    ...
                    DGLE.SetRange("Document Type", DGLE."Document Type"::"Credit Memo");
                        ...
                        repeat
                            ...
                            ELSE
                                IF DGLE."GST Component Code" = 'CGST' THEN BEGIN
                                    CGSTAmt := ABS(DGLE."GST Amount");
                                    CGSTPer := DGLE."GST %";
                                END

                                ELSE
                                    IF DGLE."GST Component Code" = 'IGST' THEN BEGIN
                                        IGSTAmt := ABS(DGLE."GST Amount");
                                        IGSTPer := DGLE."GST %";
                                    END
                        until DGLE.Next() = 0;

                    TotalAmount := "Unit Price" + CGSTAmt + SGSTAmt + IGSTAmt;
                    totalvalue := "Unit Price" + CGSTAmt + SGSTAmt + IGSTAmt;
            ...
            trigger OnAfterGetRecord() //SCMH
            var
                myInt: Integer;
            begin

                TotalAmount := 0;
                recSaleCrMemoLine.RESET;
                recSaleCrMemoLine.SETRANGE(recSaleCrMemoLine."Document No.", Header."No.");
                //recSalesInvoiceLine.SETRANGE(Type,recSalesInvoiceLine.Type::Item); //PCPL 0028 COMMENT
                IF recSaleCrMemoLine.FINDFIRST THEN
                    REPEAT
                        TotalAmount3 += recSaleCrMemoLine.Amount;
                ...
                IGSTAmount := 0;
                GLE.RESET;
                GLE.SETRANGE("Document No.", Header."No."); //Pooja PCPL-64 
                ...
                // Clear(IRNNO);
                // RecEInvoiceDetail.Reset();
                // RecEInvoiceDetail.SetRange("Document No.", RecEInvoiceDetail."E-Invoice IRN No.");
                // RecEInvoiceDetail.SetRange("Document No.", "No.");
                // if RecEInvoiceDetail.FindFirst() then begin
                //     IRNNO := RecEInvoiceDetail."E-Invoice IRN No.";
                //    // QrCode := RecEInvoiceDetail."E-Invoice QR Code";
                // end;
                ...
                // repCheck.FormatNoText(AmountinWords, TotalAmount + SGSTAmt + CGSTAmt + IGSTAmt, Header."Currency Code");

                repCheck.InitTextVariable;
                repCheck.FormatNoText(AmountinWords, Round(TotalAmount3 + IGSTAmount + CGSTAmount + SGSTAmount), Header."Currency Code");
            ...
            dataitem(Totals; "Integer")
            {
                ...
                column(TotalIncludingVATText; 'Total INR Incl. GST')
                {
                }
    ...
    var
        ...
        repCheck: Codeunit "Amount In Words";
        SerialNo: Integer;
